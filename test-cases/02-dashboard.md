...
| 21 | Log in as Admin with zero pending sign-up requests | Valid Admin account, no pending `signup_requests` rows | Tile still shows, with value "0"; hovering over it shows the normal cursor (not a pointer), and clicking it does nothing — it's a plain, non-interactive tile |
| 21a | Hover over the "Access requests" tile with 1+ pending requests | Valid Admin account, 1+ pending `signup_requests` | Cursor changes to a pointer over the tile, in addition to the existing hover lift/shadow shared by every stat tile |
| 21b | Click the "Access requests" tile with 1+ pending requests | Valid Admin account, 1+ pending `signup_requests` | Browser navigates to `/employees`; see [`10-signup-requests.md`](10-signup-requests.md) for the highlight behavior on arrival |
