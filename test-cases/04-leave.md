...
| 8a | Open the Apply/Edit dialog for a leave type with little or no balance left | Leave type with 0 remaining days | Inline warning appears under the leave type dropdown ("No Casual leave left for 2026." or "Only N day(s) of Casual leave left — reduce the date range."); Submit button is disabled |
| 9 | Submit a leave request that exceeds the remaining balance for that leave type | Leave type with 0 or few days left, request exceeding remaining days | Blocked before it can even be submitted (see #8a); a direct `POST /api/leave` call bypassing the UI is also rejected with `400` and the same message — balance can never go negative |
...
| 15a | Approve enough requests of the same leave type to exceed the employee's total for that type | Employee already near their limit, one more pending request that would push them over | Approval is rejected with a clear error (names the employee, leave type, days left, and days requested); the request stays "Pending"; the balance is not touched |
